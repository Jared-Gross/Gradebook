...
class PrimeFactorizationHandler(tornado.web.RequestHandler):
    ...
    def generate_factorization_tree_html(self, index, number, up_to) -> str:
        ...
        return build_html_tree(factors) + f'<button class="waves-effect waves-light btn" id="check" onclick="checkLeafProduct(\'{index}\')">Check</button>'
    ...
    def generate_numbers(self, count: int) -> list[int]:
        numbers: set[int] = set()
        while len(numbers) < count:
            new_number = random.randint(4, 20)
    ...
    def generate_trees(self, count):
        html_string = ""
        for i, number in enumerate(self.generate_numbers(count)):
            html_string += f'<div class="page" id=page{i+1}>{self.generate_factorization_tree_html(i+1, number, up_to=2)}</div>'
        return html_string
