...
def initialize():
    global \
        ...
        materialize_js, \
        grade_ranges
    database_location = os.path.join(os.getcwd(), "database")
    student_report_html_template = os.getcwd() + r"\static\templates\student_report.html"
    ...
    grade_ranges = []
    with open("grade_ranges.json", "r", encoding="utf-8") as file:
        grades: dict[str, int] = json.load(file)
        grade_ranges.extend((grade, letter_grade) for letter_grade, grade in grades.items())
